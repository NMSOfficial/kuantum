"""Implementation of the TabAttention architecture used for inference."""
...
class FourierFeatures:
    """Utility to generate Fourier feature encodings."""

    basis: Tensor

    def __call__(self, x: Tensor) -> Tensor:
        x_proj = (2.0 * np.pi * x.unsqueeze(-1)) @ self.basis.t()
...
class Attention(nn.Module):
    def __init__(self, dim: int, heads: int = 8, dim_head: int = 16, dropout: float = 0.0):
        super().__init__()
...
class RowColTransformer(nn.Module):
    ...
    def forward(self, x: Tensor, x_cont: Optional[Tensor] = None, mask: Optional[Tensor] = None) -> Tensor:
...
class Transformer(nn.Module):
    ...
    def forward(self, x: Tensor, x_cont: Optional[Tensor] = None) -> Tensor:
        if x_cont is not None:
            x = torch.cat((x, x_cont), dim=1)
...
class MLP(nn.Module):
    def __init__(self, dims: Sequence[int], act: Optional[nn.Module] = None):
        super().__init__()
...
class TabAttention(nn.Module):
    def __init__(
        ...
        *,
        ...
        continuous_mean_std: Optional[Tensor] = None,
        attn_dropout: float = 0.0,
        ff_dropout: float = 0.0,
        lastmlp_dropout: float = 0.0,
        cont_embeddings: str = "MLP",
        scalingfactor: float = 10,
        attentiontype: str = "col",
    ):
        super().__init__()
        assert all(map(lambda n: n > 0, categories)), "number of each category must be positive"

        self.num_categories = len(list(categories))
        ...
        if attentiontype == "col":
            ...
            raise ValueError(f"Unsupported attention type: {attentiontype}")
        ...
        if self.num_categories > 0:
            cat_fill = torch.full((self.num_categories,), 2, dtype=torch.int8)
            cat_mask_offset = F.pad(cat_fill, (1, 0), value=0)
        ...
        if self.num_continuous > 0:
            ...
            con_mask_offset = F.pad(con_fill, (1, 0), value=0)
            con_mask_offset = con_mask_offset.cumsum(dim=-1)[:-1]
        else:
            con_mask_offset = torch.zeros(0, dtype=torch.int8)
        self.register_buffer("con_mask_offset", con_mask_offset)

        self.mask_embeds_cont = nn.Embedding(max(self.num_continuous * 2, 1), self.dim)

    def forward(self, x_categ: Tensor, x_cont: Tensor, x_categ_enc: Tensor, x_cont_enc: Tensor) -> Tensor:
    ...
    @torch.no_grad()
    def encode_continuous(self, x_cont: Tensor) -> Tensor:
        outs = []
        for i in range(self.num_continuous):
            v = self.simple_MLP[i](x_cont[:, i : i + 1])
            outs.append(v.unsqueeze(1))
        return torch.cat(outs, dim=1) if outs else torch.empty((x_cont.size(0), 0, self.dim), device=x_cont.device)

    @torch.no_grad()
    def encode_categorical(self, x_categ: Tensor) -> Tensor:
        if x_categ.numel() == 0:
            return torch.empty((x_categ.size(0), 0, self.dim), device=x_categ.device)
        x = x_categ + self.categories_offset.to(x_categ.device)
